let my_number = null, logged_in_user = null

var rows = [], media_attached = false, notifications_hash = {}, stop = false, contacts = [], contact_hash={};
...
var messages = ['Hello! how can we help you?', 'Hello!', 'Thank you for using service!' ], total_messages = 0;
const trial_features = ["Schedule", "Quick Replies", "Customizable time gap", "Random Time Gap", "Batching"];
...
function suggestion_messages() {
    ...
    var footer = document.querySelector('footer._2cYbV');
    if(!footer)
        return;
...
function suggestion_popup() {
    if(!document.getElementsByClassName("modal")[0]) {
        ...
        modal_content.appendChild($($.parseHTML('<button style="background: #00C451;border-radius: 2px;padding: 8px 12px;float: right;" id="add_message_btn">Add Template</button>'))[0]);

        modal_content.appendChild($($.parseHTML('<div style="font-size: 16px;color: black;margin: 12px auto;font-weight: bold;"><span>Powered by</span> <a href="https://wawebsender.online/" target="_blank" style="color: #00C451;text-decoration: unset;font-weight: normal;"> Web Sender</a><a href="https://web-sender.blogspot.com/2021/07/how-to-reply-quickly-to-your-customers.html" style="float: right;color: #C4C4C4;text-decoration: underline;font-weight: normal;">How to use?</a></div>'))[0]);
...
function reload_mynumber(){
    ...
    // chrome.storage.local.set({my_number: null});
    ...
    var elem = document.getElementsByClassName('_3hcUV')[0];
    if(elem && elem.innerText == 'BETA')
        trackEvent('beta_version', my_number);
}
...
function init() {
    ...
    window.onload = function() {
        ...
        if(window.location.host === 'web.whatsapp.com'){
            ...
            trackEvent('temp_test', null);
            // free_trial_start_popup();
        }
    };
...
function listner(request, sender, sendResponse){
    if(request.type === 'download_group')
        ...
        messanger(request.numbers, request.message, request.time_gap, request.csv_data, request.customization, request.random_delay, request.batch_size, request.batch_gap);
    else if(request.type === 'schedule_message')
        schedule_message(request.numbers, request.message, request.time_gap, request.csv_data, request.customization, request.schedule_time, request.random_delay, request.batch_size, request.batch_gap);
...
async function help() {
    ...
    var message = 'Hi, I need help in using Web Sender';
...
async function messanger(numbers, message, time_gap, csv_data, customization, random_delay, batch_size, batch_gap ) {
    ...
    var delays = [], total_time = 0, remaining_time = 0;
    for (let i = 0; i < numbers.length; i++) {
        var curr_time_gap = time_gap;
        if(random_delay)
            curr_time_gap = getRandomNumber(2, 7);
    ...
    for (let i = 0; i < numbers.length; i++) {
        ...
        if(messanger_sending) {
            ...
            messanger_sending.appendChild($($.parseHTML('<div style="margin: auto;display: flex; width: 342px;"><span class="mail_icon"></span>Currently sending to :  <strong style="padding: 0px 12px;">' + number + '</strong>  ( ' + (i+1) + ' of ' + numbers.length + ' )</div>'))[0]);
        }
        if(customization)
            message = messages[i];
        var a = null;
        ...
        if(messanger_time_bar) {
            ...
            var remaining_time_str = "Approx. " + ((hours > 0) ? (hours+((hours > 1) ? " hours " : " hour ")) : "") + mins +((mins > 1) ? " minutes " : " minute ") + "remaining";
            messanger_time_bar.appendChild($($.parseHTML('<div style="width: 400px;color: #fff;position: absolute;text-align: center;font-weight: normal;font-size: 12px;padding: 4px;">'+ remaining_time_str +'</div>'))[0]);
            messanger_time_bar.appendChild($($.parseHTML('<div style="width: '+remaining_bar+'%;background: #357A71;height: 100%;border-radius: 16px;"></div>'))[0]);
        }
        if (!a) {
            rows.push([numbers[i], 'Invalid Number']);
            continue;
...
function messanger_popup(){
    ...
    if(!messanger_popup_div) {
        ...
        popup.appendChild(modal_content);
        modal_content.appendChild($($.parseHTML('<div class="popup_text_title"><span class="watch_icon"></span>Your messages are being sent</div>'))[0]);
...
async function schedule_message(numbers, message, time_gap, csv_data, customization, schedule_time, random_delay, batch_size, batch_gap ) {
    ...
    notifications_hash['type'] = 'send_notification';
    notifications_hash['title'] = 'Web Sender';
    notifications_hash['message'] = 'Your campaign has been scheduled for '+time12;
    sendChromeMessage(notifications_hash);
    var ctime = new Date();
        ...
        openNumberTab(number)
            .then(() => {
                setTimeout(async function() {
                    ...
                    console.log('in');
                    console.log('a');
                    console.log(document.querySelectorAll("[contenteditable='true']")[1].innerHTML);
                }, (time_gap*1000))
            })
    })
}
...
async function hasOpened() {
    let e = !0;
    console.log(document.querySelector('[data-animate-modal-popup="true"]'));
    console.log('b');
    console.log(document.querySelectorAll("[contenteditable='true']")[1].innerHTML);
...
async function waitTillWindow() {
    console.log(document.querySelector('[data-animate-modal-popup="true"]'));
    console.log(document.querySelector('[data-animate-modal-body="true"]'));
    console.log('a');
    document.querySelector('[data-animate-modal-popup="true"]') && !document.querySelector('[data-animate-modal-body="true"]')
        ...
        console.log(document.querySelector('[data-animate-modal-popup="true"]'));
        console.log(document.querySelectorAll("[contenteditable='true']")[1]);
        console.log('f');
...
async function advance_download_group() {
    contacts = [['Phone', 'Name', 'Status']];
    contact_hash = {};

    await document.querySelector("#main > header").firstChild.click();
    group_sidebar = document.querySelectorAll('span[title=You]')[0]?.parentNode?.parentNode?.parentNode?.parentNode?.parentNode?.parentNode?.parentNode;
...
async function download_group() {
    try{
        ...
        trackEvent('basic_csv_downloaded', 'track');
    }
}
...
var stop_scroll = function(){
    ...
    var group_name = document.querySelector('div[title="Search…"]')
            .parentElement.parentElement.parentElement.parentElement.children[1].firstElementChild.textContent;
    let s = "data:text/csv;charset=utf-8," + contacts.map(e => e.join(",")).join("\n");
    var o = encodeURI(s),
        tempele = document.createElement("a");
    tempele.setAttribute("href", o), tempele.setAttribute("download", group_name + ".csv"), document.body.appendChild(tempele), tempele.click()
    trackEvent('csv_downloaded', 'track');
};
...
var scrap_group = function() {
    ...
    for (let i = 0; i < contact_list.length; i++) {
        ...
        var phone, is_saved = true;
        if (contact.querySelector('img') && contact.querySelector('img').src.match(/u=[0-9]*/)) {
            phone = contact.querySelector('img').src.match(/u=[0-9]*/)[0].substring(2).replace(/[+\s]/g, '');
        } else {
            var temp = contact.querySelector('span[title]').getAttribute('title').match(/(.?)*[0-9]{3}$/);
            if(temp){
        ...
        if(phone != "")
            contact_hash[phone] = [phone, name, status];
    }
};
...
async function attachMedia(type) {
    ...
    try {
        ...
        trackEvent('attachment_sent', my_number);
    } catch (e) {
...
function free_trial_start_popup(){
    var free_trial_start_div = document.getElementsByClassName("free_trial_start_popup")[0];
    if(!free_trial_start_div) {
        var popup = document.createElement('div');
        popup.className = 'free_trial_start_popup trial_popup';
        var modal_content = document.createElement('div');
        modal_content.className = 'free_trial_start_content trial_content';
        popup.appendChild(modal_content);
        modal_content.appendChild($($.parseHTML('<div class="trial_big_title" style="margin: 28px auto 48px;text-align: center;">Congratulations</div>'))[0]);
        modal_content.appendChild($($.parseHTML('<div class="trial_title" style="width: 342px;margin: auto;display: flex;"><span class="free_trial_icon"></span>You get 7 days free trial for PREMIUM features!</div>'))[0]);
        var features_div = $($.parseHTML('<div class="trial_features"></div>'))[0];
        $.each(trial_features, function (i, feature) {
            var dom_node = $($.parseHTML('<div class="trial_feature"><span class="check_icon"></span>'+feature+'</div>'));
            features_div.appendChild(dom_node[0]);
        });
        features_div.appendChild($($.parseHTML('<a href="https://wawebsender.online/pricing" target="_blank" class="trial_feature" style="text-decoration: underline;display: inline-block;">....and much more!</a>'))[0]);
        modal_content.appendChild(features_div);
        modal_content.appendChild($($.parseHTML('<div class="trial_desc" style="margin: 16px auto 8px;text-align: center;">Note: Basic features will remain free even after trial</div>'))[0]);
        modal_content.appendChild($($.parseHTML('<div class="green_button" style="margin: 0px auto 16px;width: 100px;">Go!</div>'))[0]);

        var body = document.querySelector('body');
        body.appendChild(popup);
    }
    else
        free_trial_start_div.style.display = 'block';
}

function free_trial_reminder_popup(){
    var free_trial_reminder_div = document.getElementsByClassName("free_trial_reminder_popup")[0];
    if(!free_trial_reminder_div) {
        var popup = document.createElement('div');
        popup.className = 'free_trial_reminder_popup trial_popup';
        var modal_content = document.createElement('div');
        modal_content.className = 'free_trial_reminder_content trial_content';
        popup.appendChild(modal_content);
        modal_content.appendChild($($.parseHTML('<div class="trial_big_title" style="margin: 42px auto 28px;text-align: center;">Reminder!</div>'))[0]);
        modal_content.appendChild($($.parseHTML('<div class="trial_title" style="width: 342px;margin: auto;display: flex;"><span class="wall_clock_icon"></span>Your free trial of PREMIUM features expires in 3 days!</div>'))[0]);
        var features_div = $($.parseHTML('<div class="trial_features"></div>'))[0];
        $.each(trial_features, function (i, feature) {
            var dom_node = $($.parseHTML('<div class="trial_feature"><span class="check_icon"></span>'+feature+'</div>'));
            features_div.appendChild(dom_node[0]);
        ...
        features_div.appendChild($($.parseHTML('<a href="https://wawebsender.online/pricing" target="_blank" class="trial_feature" style="text-decoration: underline;display: inline-block;">....and much more!</a>'))[0]);
        modal_content.appendChild(features_div);
        modal_content.appendChild($($.parseHTML('<div class="trial_desc" style="margin: 16px auto 12px;text-align: center;">Note: Basic features will remain free even after trial</div>'))[0]);
        modal_content.appendChild($($.parseHTML('<div style="display: flex;"><div class="green_button" style="margin: 0px 0px 16px 76px;width: 126px;font-size: 12px;display: inline-block;background: #fff;color: rgb(30, 79, 41);">Cancel</div><a href="https://wawebsender.online/pricing" target="_blank" class="green_button" style="margin: 0px 76px 16px 28px;width: 126px;font-size: 12px;display: inline-block;">Buy Premium</a></div>'))[0]);

        var body = document.querySelector('body');
        body.appendChild(popup);
    }
    else
        free_trial_reminder_div.style.display = 'block';
}

function premium_reminder(time_gap){
    var premium_reminder_div = document.getElementsByClassName("premium_reminder_popup")[0];
    if(!premium_reminder_div) {
        var popup = document.createElement('div');
        popup.className = 'premium_reminder_popup trial_popup';
        var modal_content = document.createElement('div');
        modal_content.className = 'premium_reminder_content trial_content';
        popup.appendChild(modal_content);
        modal_content.appendChild($($.parseHTML('<div class="trial_title" style="margin: 32px auto 24px;display: flex;width: 92px;"><span class="oops_icon"></span>Oops!</div>'))[0]);
        if(time_gap) {
            modal_content.appendChild($($.parseHTML('<div class="trial_title" style="margin: auto;font-weight: bold;width: 360px;">Default time gap : 30 seconds</div>'))[0]);
            modal_content.appendChild($($.parseHTML('<div class="trial_title" style="margin: 4px auto 36px;width: 360px;">Please buy PREMIUM to randomise or customise time gap</div>'))[0]);
        }
        else {
            modal_content.appendChild($($.parseHTML('<div class="trial_title" style="margin: 0px auto 36px;width: 280px;text-align: center;">Please purchase PREMIUM to continue using that feature</div>'))[0]);
        }
        modal_content.appendChild($($.parseHTML('<div style="display: flex;"><div class="green_button" style="margin: 0px 0px 16px 76px;width: 126px;font-size: 12px;display: inline-block;background: #fff;color: rgb(30, 79, 41);">Cancel</div><a href="https://wawebsender.online/pricing" target="_blank" class="green_button" style="margin: 0px 76px 16px 28px;width: 126px;font-size: 12px;display: inline-block;">Buy Premium</a></div>'))[0]);
        var body = document.querySelector('body');
        body.appendChild(popup);
        ...
        premium_reminder_div.style.display = 'block';
}
